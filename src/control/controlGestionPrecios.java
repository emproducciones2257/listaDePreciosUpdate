...
public class controlGestionPrecios implements ActionListener, MouseListener, KeyListener {
	...
	private String quitarEncabezado(String t) {
		...
		while (escaner.hasNext()) {
			...
			if ((temp).equals("Articulo Descripción Precio")) {
				escaner.nextLine();
				escaner.nextLine();
				escaner.nextLine();
	...
	private void pruebaDelimitador(String t) throws IOException {
		...
		while (elEscaner.hasNext()) {
			...
			contador = Leido.length();
			while (c != ' ') {

				precio = precio + c;

				c = Leido.charAt(contador - 1);
				contador--;
			...
			precio = invertirPrecio(precio);
			descripcion = (Leido.replaceFirst(precio, "")).substring(1);
			descripcion = descripcion.substring(0, descripcion.length() - 1);

			preciosDocumento temp = new preciosDocumento(codigo, descripcion, getPrecioDouble(precio), indiceCatSelec.getIdCategoria());
	...
	private Double getPrecioDouble(String precio) {
		return Double.parseDouble(precio.replace(".", "").replace(",", "."));
	}

	private String invertirPrecio(String precio) {
		String cadenaInvertida = "";

		for (int i = precio.length() - 1; i >= 0; i--) {
			char caracterActual = precio.charAt(i);
			cadenaInvertida = cadenaInvertida + caracterActual;
		}

		return cadenaInvertida.substring(0, cadenaInvertida.length() - 1);
	}
